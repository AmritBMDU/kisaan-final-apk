...
class Searchpage extends SearchDelegate{
  ...
  Future<List<Data>> getuserList({String? query}) async {
    ...
    try {
      var response = await http.post(Uri.parse('${apiDomain().domain}pointhistory'),  headers: ({
      ...
      if (response.statusCode == 200) {
        ...
        if (query!= null){
          results = results.where((element) => element.qRCode!.toLowerCase().contains((query.toLowerCase()))).toList();
        }
                      ...
                      children: [
                        TableRow(
                            ...
                            children :[
                              ...
                              Padding(
                                padding: const EdgeInsets.all(8.0),
                                child: Text('Ponts',style: TextStyle(fontSize: 12,),textAlign:TextAlign.center),
                              ),
                            ]),
                              ...
                              children: [
                                TableRow(
                                    ...
                                    children :[
                                      ...
                                      Padding(
                                        padding: const EdgeInsets.all(8.0),
                                        child: Text('${dataa.qRCode}',style: TextStyle(fontSize: 12,),textAlign:TextAlign.center),
                                      ),
                                      Padding(
                                        padding: const EdgeInsets.all(8.0),
                                        child: Text('${dataa.reference}',style: TextStyle(fontSize: 12,),textAlign:TextAlign.center),
                                      ),
                                      Padding(
                                        padding: const EdgeInsets.all(8.0),
