...
class _CartPageState extends State<CartPage> {
  ...
  CartListData() async {
    ...
    try{
      final response =
      await http.post(Uri.parse('${apiDomain().domain}Cart'),
          headers: ({
  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  blockButton(
                    width: Get.width* 0.3,
                    widget: Text('Check Out',style: TextStyle(color: Colors.white),),
                    callback: (){
                      var value = {
                        "amount":Price.toString()
                      };
                      if(Price == 0.0){
                        alertBoxdialogBox(context, 'Alert', 'No Product Found');
                      }else{
      ...
      body: CarList.Items.isNotEmpty && CarList.Items != null?
              ...
              Card(
                ...
                child: ListTile(
                  leading: Container(
                      height: 50,
                      width: 50,
                      child: Image.network('${apiDomain().imageUrl + data.image.toString()}',fit: BoxFit.cover,)),
                  title: Row(
                                  ...
                                  setState(() {
                                    if(data.quantity != '1'){
                                      if(data.quantity! <= 1){
                                        ...
                                        data.quantity  =  (int.parse(data.quantity.toString())-1);
                                        getTotal();
                                        var value = {
                                          "id":data.id,
          ...
          }):Center(child: Image.asset(apiDomain().nodataimage.toString()),),
    );
  }
  double getTotal() {
    double total = 0.0;
  ...
  Future Update(Object value)async{
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    var token = prefs.getString('token');
    final response = await http.post(Uri.parse('${apiDomain().domain}updateQuantity'),body: jsonEncode(value),
        headers: ({
          'Content-Type': 'application/json; charset=UTF-8',
          'Accept': 'application/json',
          'Authorization': 'Bearer $token'
