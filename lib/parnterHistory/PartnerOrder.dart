...
class _DemosState extends State<Demos> {
  ...
  Future getaddress(String Orderid) async {
    ...
    try{
      final response =
      await http.post(Uri.parse('${apiDomain().domain}deatilbyUser'),body: jsonEncode(value),
          headers: ({
  ...
  Future CartListData(String Orderid) async {
    ...
    try{
      final response =
      await http.post(Uri.parse('${apiDomain().domain}orderDetailPartner'),body: jsonEncode(value),
          headers: ({
            'Content-Type': 'application/json; charset=UTF-8',
            'Accept': 'application/json',
            'Authorization': 'Bearer $token'
      ...
      if(response.statusCode ==200){
        ...
        OrderDetailParnters.Items = List.from(productsData)
            .map<PartnerOrderDetail>((product) => PartnerOrderDetail.fromJson(product))
            .toList();
        setState(() {
        });
      }else if(response.statusCode ==404){
  ...
  void initState() {
    ...
    Future.delayed(Duration(seconds: 1),(){
      Get.off(OrderDetailPartner(orderstatus: orderstatus,orderid: orderid,));
    });
  }
  @override
...
class OrderDetailPartner extends StatefulWidget {
  ...
  @override
  State<OrderDetailPartner> createState() => _CartPageState(orderid,orderstatus);
}
    ...
    return WillPopScope(
      ...
      child: Scaffold(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text('Total:  ',style: TextStyle(color: appcolor.black,fontWeight: FontWeight.w600),),
                        Text('₹$Price',style: TextStyle(color: appcolor.redColor,fontWeight: FontWeight.w600,fontSize: 20),),
                      ],
                    ),
                    SizedBox(height: 15,),
                    Row(
                      ...
                      children: [
                        blockButton(
                            widget: Text('Cancel',style: TextStyle(color: Colors.white),),
                            callback: (){
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        ElevatedButton(onPressed: (){
                                          ...
                                          Updatedata('orderDetailPartner', value, context);
                                        }, child: Text('Yes',style: TextStyle(color: appcolor.redColor),))
                                      ],
                                    )
                                  ],
                                );
                        ...
                        blockButton(
                          color: Colors.green,
                            widget: Text('Confirm',style: TextStyle(color: Colors.white),),
                            callback: (){
                              var value = {
                                "order_id": orderid,
                                "status":'confirm'
        ...
        appBar: AppBar(
          ...
          title: Text('Order Detail', style: TextStyle(fontSize: 20,color: Colors.black),),
        ...
        Column(
          children: [
                    ...
                    return ListView.builder(
                        ...
                        itemBuilder: (context, index){
                          ...
                          return Card(
                            ...
                            Text('${data['name']}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                              Text('${data['mobile_no']}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                              Text('${data['address']},',style: TextStyle(fontSize: 14,fontWeight: FontWeight.w500),),

                              Padding(
                                padding: const EdgeInsets.only(bottom: 5),
                                child: Text('${data['city']}, ${data['state']}, ${data['country']}, ${data['pincode']}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.w500),),
                              ),
            ...
            Expanded(
              child: OrderDetailParnters.Items.isNotEmpty && OrderDetailParnters.Items != null?
              ListView.builder(
                  itemCount: OrderDetailParnters.Items.length,
                  itemBuilder: (context, index){
                    var data =  OrderDetailParnters.Items [index];
                    var totalCost = double.parse(data.price.toString()) * double.parse(data.quantity.toString());
                    return
                      Card(
                        elevation: 4,
                        ...
                        child: ListTile(
                          leading: Container(
                              height: 50,
                              width: 50,
                              child: Image.network('${apiDomain().imageUrl +  data.image.toString()}',fit: BoxFit.contain,)),
                          title: Row(
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  SizedBox(
                                    width: Get.width * 0.4,
                                    child: Text(data.productName.toString(),style: TextStyle(
                                      fontSize: 14,
                                      height: 0.9,
                                    ),maxLines: 3,),
                                  ),
                              ...
                              Row(
                                children: [
                                  ...
                                  Text(data.size.toString(),style: TextStyle(
                                    fontSize: 14,
                                    height: 0.9,
                                  ),),
                                ],
                          ...
                          subtitle:Column(
                            children: [
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      Text(data.quantity.toString(),style: TextStyle(
                                        fontSize: 14,
                                        height: 0.9,
                                      ),),
                                    ],
                              ...
                              Row(children: [
                                Text('Code: ',style: TextStyle(color: appcolor.redColor),),
                                Text('${data.code}',style: TextStyle(color: appcolor.black),)
                              ],)
                            ],
                          ),
                        ),
  ...
  double getTotal() {
    ...
    setState(() {
      OrderDetailParnters.Items.forEach((element) {
        total += (int.parse(element.price.toString()))! * (int.parse(element.quantity.toString()));
      });
    });
    setState(() {
